...
async function callFlaskServer(prompt) {
    try {
        const response = await axios_1.default.post(FLASK_SERVER_URL, {
            prompt: prompt
        });
...
app.post("/template", async (req, res) => {
    ...
    try {
        ...
        const projectTypeResponse = await callFlaskServer(`Is the project React or Node? Determine based on the following description return in just one word nothing else: ${prompt}`);
        const projectType = (_a = projectTypeResponse.response) === null || _a === void 0 ? void 0 : _a.trim().toLowerCase();
        ...
        const basePrompt = projectType === "react" ? react_1.basePrompt : node_1.basePrompt;
        const structureResponse = await callFlaskServer(`Provide the detailed structure and complete code for a ${projectType} project with the complete functionality of the project asked in this prompt: ${prompt}`);
...
app.post("/chat", async (req, res) => {
    try {
        ...
        const combinedPrompt = messages.map(msg => `${msg.role}: ${msg.content}`).join("\n");
        const systemPromptStr = (0, prompts_1.getSystemPrompt)();
